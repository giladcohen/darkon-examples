...
class Train(object):
    ...
    def train(self):
        ...
        all_data, all_labels = prepare_train_data(padding_size=0)
        
        # find dogs
        labeled_dogs = np.where(all_labels == 5)[0]
        print(labeled_dogs)
        # shuffle dogs
        ...
        # make mislabel from dogs to horse
        all_labels[mislabel_indices] = 7.0
        ...
        if FLAGS.is_use_ckpt is True:
            ...
            print '\nRestored from checkpoint... {}'.format(FLAGS.ckpt_path)
        else:
            sess.run(init)
        ...
        for step in xrange(FLAGS.train_steps):
            ...
            # np.savetxt('train-{}.txt'.format(step), train_batch_data.reshape(-1))
    ...
    def generate_augment_train_batch(train_data, train_labels, train_batch_size):
        ...
        # print(EPOCH_SIZE - train_batch_size)
    ...
    def train_operation_wo_top(self, global_step, total_loss):
        '''
        Defines train operations
        :param global_step: tensor variable with shape [1]
        :param total_loss: tensor with shape [1]
